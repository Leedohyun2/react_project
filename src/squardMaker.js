  ...
  constructor(props) {
    ...
    this.state = {
      players: players.map((player, index) => ({
        ...player,
        defaultPosition: this.getPositionFromLocalStorage(index) || player.defaultPosition,
      })),
    };
  }

  getPositionFromLocalStorage = (index) => {
    const storedPosition = localStorage.getItem(`player-${index}`);
    return storedPosition ? JSON.parse(storedPosition) : null;
  };

  handleDragStop = (index, position) => {
    localStorage.setItem(`player-${index}`, JSON.stringify({ x: position.x, y: position.y }));
  };
